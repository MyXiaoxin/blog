...
​				SpringFactoriesLoader.loadFactoryName（**EnableAutoConfiguration**.class，**classLoader**）：SpirngBoot在启动的时候从类路径下的**META-INF/spirng.factories**中获取EnableAutoConfiguration指定的值，将这些值作为自动配置类导入到容器中，自动配置就生效，帮我们进行自动配置工作；

以前我们需要自己配置的东西，自动配置类都帮我们做了，而不是Spring不需要这些东西
...
当@ResponseBody注解放在@Controller注解的类上时，表示类中所有返回的数据直接写给浏览器，如果是对象转为Json数据
...
SpringBoot使用一个配置文件：application.properties     application.yml

配置文件的作用：修改SpringBoot的默认配置；

## 2.YML语法

k: v    **空格**是必须有的以空格的缩进来控制层级关系，左对齐的是同一级的数据

值的写法：

​	字面量：直接写就行了，不用加单引号或者双引号;;;“”不会转移字符串“zhangsan\n”;;;;单引号会转义

​	对象：还是k; v的方式，下一行写对象的属性和值，使用空格控制层级;;;{a: b,c: d}

​	数组：（List、Set）:用  -  表示一个属性  -  后面要有空格  [a,b,c]
...
| 松散绑定（松散语法） | 支持（lastName）         | 不支持（last-name） |
| SpEL                 | 不支持                   | 支持                |
| JSR303数据校验       | 支持                     | 不支持              |
| 复杂类型封装         | 支持                     | 不支持              |
...
@PropertySource:读取指定的配置文件，可以加载多个配置文件

@ImportResource：导入Spring的配置文件，让配置文件里面的内容生效；把它标在一个类上即可
...
优先级优先到低，高优先级的会覆盖低优先级的配置文件



spring.config.location=xxx;可以指定配置文件的位置
...
将类路径MATE/INF/spring.factories里面的配置的所有EnableAutoConfiguration的值加入到了容器中；

每一个这样的xxxAutoConfiguration类都是容器中的一个组件，都加入到容器中；用他们来做自动配置；

3）每一个自动配置类进行自动配置功能

4）以**HttpEncodingAutoConfiguration**为例解释自动配置的原理；
...
5）所有在配置文件中能配置的属性都是在xxxProperties类中封装；配置文件能配置什么就可以参照某个功能对应的这个属性类
...
每一个日志的实现框架都有自己的配置文件。是哪个slf4j以后，配置文件还是做成日志框架自己本身的配置文件。
...
```xml
		<dependency>
			...
			<exclusions><!--在这里排除了commons-logging-->
				<exclusion>
...
2）SpringBoot已经默认将这些场景配置好了，只要在配置文件中指定少量配置就可以运行起来
...
这个场景SpringBoot帮我们配置了什么？能不能修改？可以修改哪些配置？能不能扩展？xxx
...
jsp、Velocity、Freemarker、Thymleaf
...
```xml
现在的2.0以上的版本Thymleaf默认都用最新的，不用像视频中那么配置了
<dependency>
       <groupId>org.springframework.boot</groupId>
...
只要我们把HTML页面放在classpath:/templates/，thymeleaf就帮我们自动渲染；
...
> 注意这个地方，th属性可以直接替换掉原来的原生属性的值，如果同时存在的话
...
Spring Boot 已经自动配置好了SpringMVC，我们只有直接使用就行了：
...
```java
		@Bean
		@ConditionalOnProperty(prefix = "spring.mvc", name = "date-format")//在文件中配置日期格式化的规则,如果不配置的话事没有的  COndition注解
...
​	1）、SpringBoot在自动配置很多组件的时候，先看容器中有没有用户自己配置的（@Bean、@Component）如果有就用用户配置的，如果没有，才自动配置；如果有些组件可以有多个（ViewResolver）将用户配置的和自己默认的组合起来；**很多时候只要把组件添加进进去就行了**
